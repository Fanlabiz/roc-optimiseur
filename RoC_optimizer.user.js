...
(function () {
    ...
    function buildOptimData(prio_par_type, boost100_required_set) {
        ...
        allBuildings.forEach(function(b) {
            ...
            if(b.name.indexOf('CityHall')!==-1){
                ...
                // Valeurs manuelles (priorité absolue)
                try{
                    var _mc=parseInt(localStorage.getItem('_rocManualCityHallCulture')||'0');
                    var _mr=parseInt(localStorage.getItem('_rocManualCityHallRange')||'0');
                    if(_mc>0){culture=_mc;log('CityHall manuel: culture='+culture,'#00ff88');}
                    if(_mr>0){range=_mr;log('CityHall manuel: rayon='+range,'#00ff88');}
                }catch(e){}
                // Fallbacks si pas de valeur manuelle
                if(range===0&&md&&md.range>0)range=md.range;
                if(culture===0){
                    var _sc=STARTUP_CULTURE[b.name];
                    if(_sc&&_sc.culture>0){culture=_sc.culture;log('CityHall STARTUP_CULTURE: '+culture,'#a8e6a3');}
                    else if(window._rocCultureValue>0){culture=window._rocCultureValue;log('CityHall _rocCultureValue: '+culture,'#f0c040');}
                        ...
                        if(cv0>0&&cv0<10000){culture=cv0;window._rocCultureValue=cv0;log('CityHall WASM: '+culture,'#f0c040');}
                    }
                    if(culture===0&&b.cityDtoCulture>0){culture=b.cityDtoCulture;log('CityHall cityDtoCulture: '+culture,'#f08080');}
    ...
    function optimizeMultiswap(placed,terrain_grid,max_r,max_c,progress_cb,time_budget_ms,mode){
        ...
        for(var bi=0;bi<placed.length&&bi<saBest.length;bi++){placed[bi].r=saBest[bi].r;placed[bi].c=saBest[bi].c;placed[bi].rows=saBest[bi].rows;placed[bi].cols=saBest[bi].cols;if(saBest[bi].boost_min!==undefined)placed[bi].boost_min=saBest[bi].boost_min;}
    ...
    function buildExcelOutput(optimized, originalPlaced, terrain_grid, max_r, max_c, buildings_def, boost100_req, prio_par_type, optim_mode, protected_cats){
        ...
        // Enrichir nom_fr maintenant que LOCA est forcément chargé
        optimized.forEach(function(b){if(!b.nom_fr&&b.nom)b.nom_fr=getLocaName(b.nom)||'';});
        originalPlaced.forEach(function(b){if(!b.nom_fr&&b.nom)b.nom_fr=getLocaName(b.nom)||'';});
        function dn(b){return b.nom_fr||(b.nom&&getLocaName(b.nom))||b.nom||'';}
        ...
        ws2['B'+ri2]=mc('Avant: '+Math.round(sAv*100)/100,{font:{bold:true}});
        ws2['D'+ri2]=mc('Après: '+Math.round(sAp*100)/100,{font:{bold:true}});
        ws2['F'+ri2]=mc('Gain: '+(dl>=0?'+':'')+Math.round(dl*100)/100,{font:{bold:true,color:{rgb:dl>=0?'006400':'CC0000'}}});
    ...
    function getProducteurTypes(){
        ...
        try {
            panel.innerHTML =
                ...
                +'<div style="background:#1a1a2e;border:1px solid #3a3a5a;border-radius:6px;padding:8px;margin-bottom:8px;">'
                +'<div style="color:#a8e6a3;font-size:11px;font-weight:bold;margin-bottom:6px;">🏛️ Hôtel de ville</div>'
                +'<div style="color:#888;font-size:10px;margin-bottom:3px;">Culture de l&#39;hôtel de ville</div>'
                +'<div style="display:flex;align-items:center;gap:3px;margin-bottom:8px;">'
                +'<span data-roc="ch_cult_minus100" style="background:#1a2a3a;color:#adf;border-radius:4px;padding:5px 5px;font-size:10px;cursor:pointer;touch-action:manipulation;white-space:nowrap;">−100</span>'
                +'<span data-roc="ch_cult_minus10" style="background:#1a2a3a;color:#adf;border-radius:4px;padding:5px 5px;font-size:10px;cursor:pointer;touch-action:manipulation;white-space:nowrap;">−10</span>'
                +'<span data-roc="ch_cult_minus" style="background:#333;color:#fff;border-radius:4px;padding:5px 9px;font-size:15px;cursor:pointer;touch-action:manipulation;">−</span>'
                +'<span id="roc-ch-culture-val" style="flex:1;text-align:center;background:#0a0a1a;color:#fff;border:1px solid #4a4a6a;border-radius:4px;padding:5px 2px;font-size:13px;">'+(localStorage.getItem('_rocManualCityHallCulture')||'—')+'</span>'
                +'<span data-roc="ch_cult_plus" style="background:#333;color:#fff;border-radius:4px;padding:5px 9px;font-size:15px;cursor:pointer;touch-action:manipulation;">+</span>'
                +'<span data-roc="ch_cult_plus10" style="background:#1a2a3a;color:#adf;border-radius:4px;padding:5px 5px;font-size:10px;cursor:pointer;touch-action:manipulation;white-space:nowrap;">+10</span>'
                +'<span data-roc="ch_cult_plus100" style="background:#1a2a3a;color:#adf;border-radius:4px;padding:5px 5px;font-size:10px;cursor:pointer;touch-action:manipulation;white-space:nowrap;">+100</span>'
                +'</div>'
                +'<div style="color:#888;font-size:10px;margin-bottom:3px;">Rayonnement de l&#39;hôtel de ville</div>'
                +'<div style="display:flex;align-items:center;gap:4px;">'
                +'<span data-roc="ch_range_minus" style="background:#333;color:#fff;border-radius:4px;padding:5px 9px;font-size:15px;cursor:pointer;touch-action:manipulation;">−</span>'
                +'<span id="roc-ch-range-val" style="flex:1;text-align:center;background:#0a0a1a;color:#fff;border:1px solid #4a4a6a;border-radius:4px;padding:5px;font-size:13px;">'+(localStorage.getItem('_rocManualCityHallRange')||'—')+'</span>'
                +'<span data-roc="ch_range_plus" style="background:#333;color:#fff;border-radius:4px;padding:5px 9px;font-size:15px;cursor:pointer;touch-action:manipulation;">+</span>'
                +'</div>'
                +'</div>'
                +'<div style="color:#a8e6a3;font-size:11px;font-weight:bold;margin:6px 0 2px;">📊 Priorités</div>'
    ...
    function startCountdown(){
        ...
        // Les valeurs CityHall sont déjà dans localStorage via les boutons +/-
        ...
        setTimeout(function(){
            ...
            if(modalMode!=='none'){
                ...
                placed.forEach(function(b){
                    ...
                    if(!cat||!prot[cat])return;
                    var c=cultureReceived(b,cults0);
                    var bl0=boostLevel(c,b);
                    if(bl0>0) b.boost_min=bl0; // Ne protéger que si boost > 0
                });
            }
    ...
    function renderContent(panel){
        ...
        var html='<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">'
            +'<strong style="color:#a8e6a3;font-size:13px;">🏙️ RoC Optimiseur v6.5</strong>'
    ...
    (function() {
        function _rocDocHandler(e) {
            var t = e.target;
            // Ne pas intercepter les inputs et selects (saisie clavier iPad)
    ...
    window._roc = {
        ...
        ch_cult_plus: function(){
            try{var el=document.getElementById('roc-ch-culture-val');var v=parseInt(localStorage.getItem('_rocManualCityHallCulture')||'0')||0;v+=1;localStorage.setItem('_rocManualCityHallCulture',v);if(el)el.textContent=v;}catch(e){}
        },
        ch_cult_plus10: function(){
            try{var el=document.getElementById('roc-ch-culture-val');var v=parseInt(localStorage.getItem('_rocManualCityHallCulture')||'0')||0;v+=10;localStorage.setItem('_rocManualCityHallCulture',v);if(el)el.textContent=v;}catch(e){}
        },
        ch_cult_plus100: function(){
            try{var el=document.getElementById('roc-ch-culture-val');var v=parseInt(localStorage.getItem('_rocManualCityHallCulture')||'0')||0;v+=100;localStorage.setItem('_rocManualCityHallCulture',v);if(el)el.textContent=v;}catch(e){}
        },
        ch_cult_minus100: function(){
            try{var el=document.getElementById('roc-ch-culture-val');var v=parseInt(localStorage.getItem('_rocManualCityHallCulture')||'0')||0;if(v>100)v-=100;else v=1;localStorage.setItem('_rocManualCityHallCulture',v);if(el)el.textContent=v;}catch(e){}
        },
        ch_cult_minus: function(){
            try{var el=document.getElementById('roc-ch-culture-val');var v=parseInt(localStorage.getItem('_rocManualCityHallCulture')||'0')||0;if(v>1)v-=1;localStorage.setItem('_rocManualCityHallCulture',v);if(el)el.textContent=v;}catch(e){}
        },
        ch_cult_minus10: function(){
            try{var el=document.getElementById('roc-ch-culture-val');var v=parseInt(localStorage.getItem('_rocManualCityHallCulture')||'0')||0;if(v>10)v-=10;else v=1;localStorage.setItem('_rocManualCityHallCulture',v);if(el)el.textContent=v;}catch(e){}
        },
        ch_range_plus: function(){
            try{
                var el=document.getElementById('roc-ch-range-val');
                var v=parseInt(localStorage.getItem('_rocManualCityHallRange')||'0')||0;
                if(v<10)v+=1; localStorage.setItem('_rocManualCityHallRange',v);
                if(el)el.textContent=v;
            }catch(e){}
        },
        ch_range_minus: function(){
            try{
                var el=document.getElementById('roc-ch-range-val');
                var v=parseInt(localStorage.getItem('_rocManualCityHallRange')||'0')||0;
                if(v>1)v-=1; localStorage.setItem('_rocManualCityHallRange',v);
                if(el)el.textContent=v;
            }catch(e){}
        },
