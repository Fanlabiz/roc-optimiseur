...
(function () {
    ...
    function buildExcelOutput(optimized, originalPlaced, terrain_grid, max_r, max_c, buildings_def, boost100_req){
        ...
        while(remaining.length>0&&maxIter-->0){
            ...
            // Si aucun déplacement possible (cycle) : mettre en réserve temporairement
            if(!progress&&remaining.length>0){
                var mv0=remaining.shift();
                // Insérer un déplacement vers la réserve avant ce bâtiment
                ...
                // Le bâtiment sera replacé depuis la réserve plus tard
        ...
        if(sorted.length===0){ws3['A2']=mc('Aucun deplacement.',{});}
            ...
            var from_str = mv.reserve    ? coordR(mv.from[0],mv.from[1])
                         : mv.fromReserve? '⭐ Réserve'
                         : coordR(mv.from[0],mv.from[1]);
            var to_str   = mv.reserve    ? '⭐ Réserve (temporaire)'
                         : mv.fromReserve? coordR(mv.to[0],mv.to[1])
                         : coordR(mv.to[0],mv.to[1]);
    ...
    function renderContent(panel){
        ...
        var html='<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">'
            +'<strong style="color:#a8e6a3;font-size:13px;">🏙️ RoC Optimiseur v6.3</strong>'
