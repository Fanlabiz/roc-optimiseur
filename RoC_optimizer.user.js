...
(function () {
    ...
    function buildOptimData(prio_par_type, boost100_required_set) {
        ...
        allBuildings.forEach(function(b) {
            ...
            }else if(isCT&&(culture===0||range===0)){
                var cr=computeCultureRay(b.name,level,era);
                if(cr&&cr[0]>0)culture=cr[0];if(cr&&cr[1]>0)range=cr[1];
            }
        ...
        realEnriched.forEach(function(b){
            ...
            placed.push(Object.assign({},bdef,{nom:b.name,nom_fr:b.nom_fr,r:b.row-gLM,c:b.col-gCM,rows:b.h,cols:b.w,culture:b.culture,rayonnement:b.range,era:b.era}));
        });
    ...
    function scorePlacement(placed){
        ...
        for(var i=0;i<placed.length;i++){
            ...
            if(b.boost_min!==undefined&&bl<b.boost_min)total-=50000;
        }
        return total;
    }
    ...
    function buildExcelOutput(optimized, originalPlaced, terrain_grid, max_r, max_c, buildings_def, boost100_req){
        ...
        function buildListe(placed,cults){
            var ws={};
            var hdrs=['Nom','Type','Placé','Coord','Orient','Priorité','Placement',
            ...
            .forEach(function(b){
                ...
                var vals=[dn(b),b.type,'Oui',coordR(b.r,b.c),b.cols>=b.rows?'H':'V',
            ...
            ws['!cols']=[32,12,6,10,7,8,11,13,7,9,9,10,13,12,11].map(function(w){return{wch:w};});
            return ws;
        }
        ...
        optimized.forEach(function(b){
            ...
            [++mvIdx,dn(b),coordR(op[0],op[1]),coordR(b.r,b.c)].forEach(function(v,i){ws3[colLetter(i+1)+ri3]=mc(v,S.move());});
            ri3++;
        });
        if(ri3===2)ws3['A2']=mc('Aucun deplacement.',{});
    ...
    function getProducteurTypes(){
        ...
        var protectHtml='<div style="color:#a8e6a3;font-size:11px;font-weight:bold;margin:6px 0 2px;">Protéger (sans réduction)</div>';
    ...
    function runOptimization(){
        ...
        setTimeout(function(){
            ...
            // En mode no_reduction : calculer les boosts initiaux pour les catégories protégées
            if(modalMode==='no_reduction'){
                var cults0=placed.filter(function(b){return b.type==='Culturel';});
                var prot=window._rocProtected||{Barracks:true,Farm:true,Home:true,Workshop:true};
                placed.forEach(function(b){
                    if(b.type!=='Producteur')return;
                    var cat=csvBuildingCategory(b.nom||b.name||'');
                    if(!cat||!prot[cat])return; // catégorie non protégée
            ...
            if(modalMode==='none'){
                finishOptim(placed, originalPlaced, data, boost100_req, show);
                return;
            }
            ...
            setTimeout(function(){
                ...
                finishOptim(optimized, originalPlaced, data, boost100_req, show);
            }, 100);
        }, 100);
    }
    function finishOptim(optimized, originalPlaced, data, boost100_req, show){
    ...
    function renderContent(panel){
        ...
        var html='<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">'
            +'<strong style="color:#a8e6a3;font-size:13px;">🏙️ RoC Optimiseur v6.0</strong>'
