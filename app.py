...
def optimize_multiswap(placed, terrain_grid, max_r, max_c,
    ...
    def _prog(frac, phase=""):
        if progress_cb:
            progress_cb(min(frac, 0.98), phase)
    ...
    if mode == "no_reduction" and protected_categories:
        ...
        for b in _ref:
            ...
            nom = b.get("nom", "")
            # Prendre le minimum par nom (le moins bien couvert initialement)
    ...
    _prog(_P_GREEDY, "Passe greedy terminée")
    ...
    # ── Phase 1 : swaps de 3 ──────────────────────────────────────────────
    _t_sw3_end = _T_GREEDY + _T_SW3
    improved_any = True
    while improved_any and _elapsed() < _t_sw3_end:
        improved_any = False
        cands = _candidates()
        n_cands = len(cands)
        score_before = _score()
    ...
    _prog(_P_SW3, "Swaps de 3 terminés")
    ...
    while improved_any and _elapsed() < _T_GREEDY + _T_SW3 + _T_SW4:
        ...
        _prog(_P_SW3 + frac_sw4 * (_P_SW4 - _P_SW3),
              f"Swaps de 4 — {frac_sw4*100:.0f}%")

    _prog(_P_SW4, "Swaps de 4 terminés")
    ...
    if _ratio < 0.60 and _top_prods and _remaining() > 5:
        ...
        while _elapsed() < t_perturb_end:
            frac_p = min((_elapsed() - _T_GREEDY - _T_SW3 - _T_SW4) / max(_T_PERTURB, 1), 1.0)
            _prog(_P_SW4 + frac_p * (_P_PERTURB - _P_SW4), f"Perturbation — {frac_p*100:.0f}%")
    ...
    _prog(_P_PERTURB, "Perturbation terminée")
...
if 'placed' in dir() and 'original_placed' in dir():
    ...
    if st.button("Lancer l'optimisation", type="primary"):
        ...
        _t_optim_start = _t_mod.time()
        _phase_log = [""]

        def update_prog(v, phase=""):
            progress_bar.progress(v)
            elapsed = _t_mod.time() - _t_optim_start
            msg = phase if phase else "en cours"
            status.info(f"Optimisation en cours — {msg} ({elapsed:.0f}s écoulées)")
