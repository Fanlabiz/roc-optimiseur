...
def optimize_multiswap(placed, terrain_grid, max_r, max_c,
    ...
    # _snap_reference est calculé en amont (avant Phase 0) pour être actif dès le début.

    def _no_regression(snap_before, snap_after):
        """Aucun producteur PROTÉGÉ n'a perdu de palier par rapport à :
        - son boost dans snap_before (état avant le mouvement courant), ET
        - son boost dans _snap_reference (état initial / reference_placed).
        Les bâtiments boost100_required ne peuvent jamais descendre sous 100%."""
        for b in placed:
            if b["type"] != "Producteur": continue
            bid = id(b)
            boost_after = snap_after.get(bid, 0)
            # Contrainte boost100_required
            if b.get("boost100_required", False):
                if boost_after < 100:
                    return False
            # Contrainte no_reduction
            if not _is_protected(b): continue
            # Ne pas régresser par rapport au snapshot avant le mouvement
            if boost_after < snap_before.get(bid, 0):
                return False
            # Ne pas régresser par rapport à l'état de référence initial
            nom = b.get("nom", "")
            if nom in _snap_reference and boost_after < _snap_reference[nom]:
                return False
        return True
    ...
    # En mode no_reduction, le plancher de référence est soit l'état initial
    # (reference_placed fourni, ex: original_placed en mode terrain vide),
    # soit l'état courant de placed_for_optim au démarrage.
    # _snap_reference[nom] = boost minimal à ne pas dépasser (indexé par nom
    # car les id() changent entre reference_placed et placed).
    # DOIT être calculé AVANT la Phase 0 pour que _no_regression soit actif dès le début.
    _snap_reference = {}  # nom_complet → boost de référence
    if mode == "no_reduction" and protected_categories:
        _ref = reference_placed if reference_placed is not None else placed
        _ref_cults = [b for b in _ref if b["type"] == "Culturel"]
        for b in _ref:
            if b["type"] != "Producteur": continue
            cat = _csv_building_category(b.get("nom_complet", b.get("nom", "")))
            if cat not in protected_categories: continue
            bl = boost_level(culture_received(b, _ref_cults), b)
            nom = b.get("nom", "")
            _snap_reference[nom] = min(_snap_reference.get(nom, 9999), bl)
    ...
    def _greedy_pass_safe():
        ...
        for b in _sorted:
            ...
            # Vérification anti-chevauchement: b et _moved_neutre ne doivent pas
            # chevaucher d'autres bâtiments dans placed.
            _overlap_detected = False
            _b_cells = {(b["r"]+dr, b["c"]+dc)
                        for dr in range(b["rows"]) for dc in range(b["cols"])}
            for _other in placed:
                if _other is b: continue
                if _moved_neutre is not None and _other is _moved_neutre: continue
                _o_cells = {(_other["r"]+dr, _other["c"]+dc)
                            for dr in range(_other["rows"]) for dc in range(_other["cols"])}
                if _b_cells & _o_cells:
                    _overlap_detected = True
                    break
            if not _overlap_detected and _moved_neutre is not None:
                _n_cells = {(_moved_neutre["r"]+dr, _moved_neutre["c"]+dc)
                            for dr in range(_moved_neutre["rows"])
                            for dc in range(_moved_neutre["cols"])}
                for _other in placed:
                    if _other is _moved_neutre or _other is b: continue
                    _o_cells = {(_other["r"]+dr, _other["c"]+dc)
                                for dr in range(_other["rows"])
                                for dc in range(_other["cols"])}
                    if _n_cells & _o_cells:
                        _overlap_detected = True
                        break

            # Annuler si chevauchement détecté
            if _overlap_detected:
                ...
                if all_moves and all_moves[-1].get("nom") == b["nom"] \
                        and all_moves[-1].get("old_r") == _old[0] \
                        and all_moves[-1].get("old_c") == _old[1]:
                    all_moves.pop()
                elif len(all_moves) >= 2 \
                        and all_moves[-2].get("old_r") == _old[0] \
                        and all_moves[-2].get("old_c") == _old[1]:
    ...
    def _try_swap3(a, b, c, score_before, snap_before):
        ...
        if best_cfg:
            ...
            # Reconstruire _occ depuis placed pour garantir la cohérence
            _occ.clear()
            _occ.update(make_occ_grid(placed, max_r, max_c))
    ...
    t_perturb_end = _T_GREEDY + _T_SW3 + _T_SW4 + _T_PERTURB
    ...
    if _ratio < 0.60 and _top_prods and _remaining() > 5:
        ...
        while _elapsed() < t_perturb_end:
            ...
            for _cult in sorted(_inutiles,
                ...
                _cult_save_r    = _cult["r"]
                _cult_save_c    = _cult["c"]
                _cult_save_rows = _cult["rows"]
                _cult_save_cols = _cult["cols"]
                for _r in range(max_r):
                    for _c in range(max_c):
                        for _rw, _cw in _orientations(_cult):
                            ...
                            # Restaurer depuis les valeurs sauvegardées AVANT la boucle
                            _cult["r"], _cult["c"] = _cult_save_r, _cult_save_c
                            _cult["rows"], _cult["cols"] = _cult_save_rows, _cult_save_cols
                if best_pos2:
...
def build_excel_output(optimized, original_placed, terrain_grid, max_r, max_c, buildings_def,
    ...
    # Largeurs en unités openpyxl (pixels / 7.5)
    # A=130px, B=46px, C=58px, D-K=54px
    widths_boost = [17.3, 6.1, 7.7, 7.2, 7.2, 7.2, 7.2, 7.2, 7.2, 7.2, 7.2]
...
if uploaded_csv is not None:
    _active_name = getattr(uploaded_csv, "name", str(uploaded_csv))
...
if 'placed' in dir() and 'original_placed' in dir():
    ...
    if st.button("Lancer l'optimisation", type="primary"):
        ...
        _protected_cats = set()  # initialisé ici pour être accessible dans tous les modes
        if _mode_optim == "Avec réduction possible":
            ...
            optimized, moves = optimize_multiswap(
                ...
                reference_placed=original_placed,  # référence = état initial du CSV
            )
        ...
        if _boost100_failures:
            _b100_prods = [_b for _b in optimized
                           if _b.get("boost100_required") and _b["type"] == "Producteur"]
            _total_seuil = sum(_b.get("boost100", 0) for _b in _b100_prods)
            _total_culture_dispo = sum(
                _b["culture"] for _b in optimized
                if _b["type"] == "Culturel" and _b.get("rayonnement", 0) > 0
            )
            _impossible = _total_culture_dispo < _total_seuil
            if _impossible:
                _extra = (
                    f"\n\n**Raison :** la culture totale disponible "
                    f"(**{_total_culture_dispo:,}**) est inférieure à la somme des seuils 100% "
                    f"(**{_total_seuil:,}**). Il est **mathématiquement impossible** "
                    f"d'atteindre 100% pour tous ces bâtiments simultanément. "
                    f"Réduisez le nombre de bâtiments avec priorité 'max'."
                )
            else:
                _extra = (
                    "\n\n**L'optimisation est annulée.** "
                    ...
                    "Vérifiez que suffisamment de culturels sont disponibles."
                )
            st.error(
