...
def optimize_multiswap(placed, terrain_grid, max_r, max_c,
    ...
    # ── Phase 2 : swaps de 4 ──────────────────────────────────────────────
    improved_any = True
    _t_sw4_end = _T_GREEDY + _T_SW3 + _T_SW4
    while improved_any and _elapsed() < _t_sw4_end:
        improved_any = False
        cands = _candidates()
        n_cands = len(cands)
        score_before = _score()
                ...
                for k in range(j+1, n_cands):
                    ...
                    for l in range(k+1, n_cands):
                        ...
                        if _try_swap4(a, b, c_bat, d, score_before, snap_before):
                            ...
                            snap_before  = _boosts_snap() if mode == "no_reduction" else {}
                            improved_any = True
                    # Mise à jour de la barre à chaque tranche de k (toutes les n_cands itérations)
    ...
    if _ratio < 0.60 and _top_prods and _remaining() > 5:
        ...
        while _elapsed() < t_perturb_end:
            ...
            for _cult in sorted(_inutiles,
                ...
                # Sauvegarder la position initiale AVANT de la modifier
                _cult_saved_r   = _cult["r"]
                _cult_saved_c   = _cult["c"]
                _cult_saved_rows = _cult["rows"]
                _cult_saved_cols = _cult["cols"]
                for _r in range(max_r):
                    for _c in range(max_c):
                        for _rw, _cw in _orientations(_cult):
                            ...
                            # Restaurer depuis les valeurs sauvegardées (pas depuis _save!)
                            _cult["r"], _cult["c"] = _cult_saved_r, _cult_saved_c
                            _cult["rows"], _cult["cols"] = _cult_saved_rows, _cult_saved_cols
                if best_pos2:
